...
const initMulter = () => {
    ...
    const multipart = multer({
        ...
        storage: multer.diskStorage({
            ...
            //  req는 요청정보, file은 최종적으로 업로드된 결과 데이터가 저장되어있을 객체
            destination: (req, file, callback) => {

                // file 파라미터 형식은 다음과 같다.
                /*
                    file = {
                        fieldname: 'myphoto',
                ...
                */
                ...
                // 업로드 정보에 백엔드의 업로드 파이 저장 폴더 위치를 추가한다.
            ...
            filename: (req, file, callback) => {
                ...
                */
                ...
                const exName = extname(file.originalname).toLowerCase();
                // 파일이 저장될 이름(현재_시각의_timestamp + 확장자)
                const saveName = new Date().getTime().toString() + exName;
    ...
    if (err) {
        if (err instanceof multer.MulterError) {
            switch (err.code) {
                ...
                default:
                    err.result_code = 500;
                    err.result_msg = '알 수 업는 에러가 발생했습니다.';
                    break;
            }
        }
        logger.error(err);
